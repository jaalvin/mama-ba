  ...
  return (
    <div className="px-4 py-6 md:px-6 max-w-lg mx-auto flex flex-col gap-8">
      <section className="flex flex-col items-center text-center">
        ...
        <h1 className="font-headline text-headline-lg text-primary">{user?.name || "Your account"}</h1>
        <p className="text-on-surface-variant">{user?.email}</p>
      </section>

      <section>
        <h2 className="text-label-md text-on-surface-variant uppercase tracking-wider mb-2 px-1">Support &amp; Safety</h2>
        <div className="flex flex-col divide-y divide-outline-variant bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden">
          <button className="flex items-center justify-between px-4 py-4 text-left hover:bg-surface-container-low transition-colors">
      ...
      <section>
        <h2 className="text-label-md text-on-surface-variant uppercase tracking-wider mb-2 px-1">Account</h2>
        <div className="flex flex-col divide-y divide-outline-variant bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden">
          <button className="flex items-center justify-between px-4 py-4 text-left hover:bg-surface-container-low transition-colors">
          ...
          <button
            ...
            className="flex items-center justify-between px-4 py-4 text-left hover:bg-error-container/40 transition-colors"
          >
